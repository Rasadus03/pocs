assumptions:
    1) PFP provisioned EAP6.* environment with ha.group and sy-core-group 

    2) PFP provisioned EAP6.* environment is configured with an additional 'passive' backup JVM as part of the ha.group
       as per the following in $PFP_HOME/build.properties:

        ha.server.number.list=0,1

    3) EAP6.* environment is active
